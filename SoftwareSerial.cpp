...
{ 
  return 0;//_receivePortRegister->reg & digitalPinToBitMask(_receivePin);
}
...
//  PortGroup * port = digitalPinToPort(tx);
  GPIO_TypeDef* port = digitalPinToPort(tx); 
 _transmitPortRegister = (volatile PORT_OUT_Type*)portOutputRegister(port);
  //port = (volatile*)portOutputRegister(port);
}
...
//  PortGroup * port = digitalPinToPort(rx);
  GPIO_TypeDef* port = digitalPinToPort(rx);
